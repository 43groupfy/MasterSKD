...
// Gaya CSS untuk teks soal agar rapi dan tidak meluber (dipakai di card bank soal & card terpilih)
...
// ── Level 1: Card soal terpilih yang bisa di-drag untuk diurutkan ──
function SortableQuestionCard({ id, q, index, total, isBeingSwapped, dragDisabled, onMoveUpDown, onMoveToPosition, onToggleSwap, onRemove }) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    ...
    >
      {/* Drag handle — Level 1: drag & drop manual */}
      <div
        {...attributes}
        {...listeners}
        title={dragDisabled ? 'Selesaikan mode swap dulu' : 'Tahan & geser untuk mengurutkan'}
        style={{
          cursor: dragDisabled ? 'not-allowed' : (isDragging ? 'grabbing' : 'grab'),
          color: dragDisabled ? 'var(--gray-200)' : 'var(--gray-300)',
          fontSize: '16px',
          lineHeight: 1,
          flexShrink: 0,
          touchAction: 'none',
      ...
      {/* Nomor urut — Level 2: klik untuk pindah ke posisi spesifik (prompt) */}
      <div
        onClick={() => onMoveToPosition(index)}
...
export default function SelectQuestionsModal({ packageId, totalQuestions, onClose, onSaved }) {
  ...
  // Sensor untuk drag & drop (Level 1).
  // activationConstraint mencegah klik tombol lain (🔼/🔢/🔄/❌) ke-trigger sebagai drag.
  ...
  // Level 1: dipanggil dnd-kit saat drag selesai
                  ...
                  <SortableContext items={selectedIds} strategy={verticalListSortingStrategy}>
                    {selectedQuestions.map((q, index) => (
                      <SortableQuestionCard
                        ...
                        isBeingSwapped={swapIndex === index}
                        dragDisabled={swapIndex !== null}
                        onMoveToPosition={handleMoveToPosition}
                        onToggleSwap={(i) => setSwapIndex(swapIndex === i ? null : i)}
