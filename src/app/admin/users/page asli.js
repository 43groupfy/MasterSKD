...
export default function Users() {
  ...
  const fetchUsers = async () => {
    ...
    // Ambil semua user_profiles
    const { data: profiles, error: pErr } = await supabase
      .from('user_profiles')
      .select('id, status, membership, created_at, updated_at')
    ...
    // Gabungkan — email kita simpan di raw_user_meta_data lewat trigger,
    // tapi karena kita tidak bisa query auth.users langsung dari client,
    // kita pakai RPC atau simpan email di user_profiles.
    // Solusi pragmatis: ambil dari user_stats.user_id dan tampilkan ID saja,
    // lalu enrich dengan email dari sisi server jika ada.
    // Untuk sekarang: tampilkan data dari user_profiles + stats
    ...
    <AdminShell title="Pengguna" subtitle="Kelola">
      ...
      <div className="filter-bar">
        <div className="search-input-wrap" style={{ flex: 1 }}>
          ...
          <input type="text" placeholder="Cari user ID..." className="input input-search"
