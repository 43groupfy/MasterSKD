...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [levelRes, progressRes, historyRes, subRes, premiumStatus, seenIdSet] = await Promise.all([
        ...
        getSeenQuestionIdSet(user.id),
      ]);
      setIsPremium(premiumStatus.isActive);
      setSeenCount(seenIdSet.size);
      setQuotaExhausted(isQuotaExhausted(seenIdSet, premiumStatus.isActive));
        ...
        <div className="container">
          ...
          {!isPremium && (
            <a href="/premium" style={{ textDecoration: "none" }}>
              <div className="card card--hover" style={{ background: quotaExhausted ? "var(--yellow-light)" : "var(--gray-50)", border: quotaExhausted ? "2px solid var(--yellow-dark)" : "2px solid var(--gray-200)", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px", flexWrap: "wrap", gap: "8px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  <span style={{ fontSize: "1.4rem" }}>{quotaExhausted ? "🔒" : "⭐"}</span>
                  <div>
                    <div style={{ fontFamily: "var(--font-display)", fontWeight: 800, color: quotaExhausted ? "var(--yellow-dark)" : "var(--gray-800)", fontSize: "0.9rem" }}>
                      {quotaExhausted ? "Kuota Latihan Gratis Habis" : "Upgrade ke Premium"}
                    </div>
                    <div style={{ fontSize: "0.78rem", color: "var(--gray-600)" }}>
                      Kuota Latihan Gratis: {seenCount}/{FREE_QUESTION_LIMIT} soal (total lintas semua materi)
                    </div>
                  </div>
                </div>
                  ...
                  >
                    <div style={{ overflow: "hidden" }}>
                      <div style={{ borderTop: `2px solid ${info.bg}`, padding: "12px" }}>
                        ...
                        <button
                          onClick={() => {
                            if (quotaExhausted) { setShowUpgrade(true); return; }
                            router.push(`/quiz/${labelCode}?sub=all`);
                          }}
