...
export default function QuizPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Load user progress
      const [progressRes, levelRes, premiumStatus, seenIdSet] = await Promise.all([
    ...
    <>
      ...
      <main className="page-main">
        <div className="container" style={{ maxWidth: "680px" }}>
          ...
          {/* Indikator kuota latihan gratis (global, lintas semua materi) */}
          {!isPremium && (
            <div
              className="card"
              style={{
                marginBottom: "24px",
                background: quotaExhausted ? "var(--yellow-light)" : "var(--gray-50)",
                border: quotaExhausted ? "2px solid var(--yellow-dark)" : "2px solid var(--gray-200)",
                display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px",
              }}
            >
              <div style={{ fontSize: "0.85rem", color: "var(--gray-700)" }}>
                {quotaExhausted ? "🔒 " : "📚 "}
                Kuota Latihan Gratis: <strong>{seenCount}/{FREE_QUESTION_LIMIT}</strong> soal
              </div>
              {quotaExhausted && (
                <button onClick={() => setShowUpgrade(true)} className="btn btn--yellow btn--sm">⭐ Upgrade</button>
              )}
            </div>
          )}
